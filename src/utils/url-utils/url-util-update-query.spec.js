...
describe("updateQuery", () => {
  ...
  it("handles props with history and one new query", () => {
    ...
    expect(history.replace).toBeCalledWith({ search: "?test=1" });
  });
  it("handles props with history and several new queries", () => {
    const props = { history };
    updateQueryTest(props, { test: 1, val: "abc", string: "a b c" });
    expect(history.replace).toBeCalledWith({ search: "?test=1&val=abc&string=a%20b%20c" });
  });
  it("handles history push", () => {
    ...
    expect(history.replace).not.toBeCalled();
    expect(history.push).toBeCalledWith({ search: "?test=1" });
  });
  it("can replace URL search parameters (removing old ones)", () => {
    const props = { history };
    updateQueryTest(props, { a: 100, b: 50 });
    expect(history.replace).toBeCalledWith({ search: "?a=100&b=50" });
    updateQueryTest(props, { a: 50 });
    expect(history.replace).toBeCalledWith({ search: "?a=50" });
  });
